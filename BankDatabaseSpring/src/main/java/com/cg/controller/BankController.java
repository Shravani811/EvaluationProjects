...
public class BankController {
	...
	public int deposit(@RequestBody int amt) {
		int amount = 0;
		if(accountNo != 0) {
			amount = transService.deposit(accountNo,amt);
		}
		return amount;
	}
	...
	public int withdraw(@RequestBody int amt) {
		int amount = 0;
		if(accountNo != 0) {
			amount = transService.withdraw(accountNo,amt);
		}
	...
	public int showBalance() {
		int amount = 0;
		if(accountNo != 0) {
			amount = transService.showBalance(accountNo);
		}
		return amount;
	...
	public int fundTransfer(@RequestBody TransactionDetails transaction) {
		int amount = 0;
		if(accountNo != 0) {
			transaction = transService.fundTransfer(accountNo,transaction);
			boolean isInserted = transService.insertTransaction(transaction);
		}
